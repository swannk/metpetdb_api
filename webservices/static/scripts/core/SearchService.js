...
(function(s2s, $) {
	...
	s2s.SearchService = function(service, config, panels, serviceDiv)
	{
		...
		this.panels.push(new s2s.utils.FacetPanel(config));
		if (panels != null) {
		...
		var callback = function(response)
		{
			...
			{
				self.panels[i].setParent(self);
				//initialize panel from state object
				if (self.panels[i].generate && self.panelStates != null && self.panelStates[i] != null) 
				{
					self.panels[i].generate(self.panelStates[i]);
				}
				//initialize panel by default
				else if (self.panels[i].generate)
				{
					self.panels[i].generate();
				}
	...
	s2s.SearchService.prototype.initPanels = function(div) {
		var self = this;
		var facetPanel = this.getFacetPanel();
