...
edu.rpi.tw.sesf.s2s.widgets.ContextSelect.prototype.getState = function()
{
	return this.state;
}

edu.rpi.tw.sesf.s2s.widgets.ContextSelect.prototype.setState = function(state)
{
	var self = this;
...
edu.rpi.tw.sesf.s2s.widgets.ContextSelect.prototype.get = function() {
	return this.div;
}

edu.rpi.tw.sesf.s2s.widgets.ContextSelect.prototype.reset = function() {
...
edu.rpi.tw.sesf.s2s.widgets.ContextSelect.prototype.update = function(data) {
	...
	for (var i = 0; i < data.length; ++i) {
	    ...
	    var input = jQuery("<span width=\"100%\" class=\"x-option\"><input type=\"hidden\" value=\"" + item['id'] + "\" /><span class=\"x-option-label\" style=\"overflow:hidden\">" + label + "</span><a target=\"_blank_\" href=\"" + item['context'] + "\"><img style=\"height:2ex;vertical-align:text-top\" src=\"" + infoButtonImg + "\"></a></span>");
	    var self = this;
...
edu.rpi.tw.sesf.s2s.widgets.ContextSelect.prototype.updateData = function(clicked,noprop) {
...
edu.rpi.tw.sesf.s2s.widgets.ContextSelect.autocompleteSource = function(term, callback)
{
    var t = term;
    var input = this.element;
    var options = jQuery(input).parent().find(".x-checkbox");
    var data = [];
    for (var i = 0; i < options.length; ++i)
	{
	    var o = jQuery(options[i]);
	    if (o.parent().children("a").html().toLowerCase().indexOf(term.term.toLowerCase()) > -1)
		{
		   data.push({"label":o.parent().children("a").html(),"value":"","option":o[0]});
		}
	}
    callback(data);
}

edu.rpi.tw.sesf.s2s.widgets.ContextSelect.autocompleteSelect = function(event, ui)
{
    var item = ui.item;
    item.option.checked = true;
    jQuery(item.option).change();
    event.target.value = '';
