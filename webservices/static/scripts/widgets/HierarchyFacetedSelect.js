...
edu.rpi.tw.sesf.s2s.widgets.HierarchyFacetedSelect.prototype.updateClick = function(elem)
{
    ...
    if (jQuery(jQuery(li).find("input")[0]).hasClass("x-selected"))
    {
        ...
        jQuery(li).attr("style","background:#FFFFFF;border:none");	
...
edu.rpi.tw.sesf.s2s.widgets.HierarchyFacetedSelect.autocompleteSource = function(term, callback)
{
    ...
    for (var i = 0; i < options.length; ++i)
	{
		...
		{
		    data.push({"label":jQuery(o).find("span").html(),"value":"","option":jQuery(o).find("span")[0]});
		}
	}
    callback(data);
}
    ...
    if (tree != null)
    {
		for (var i = 0; i < tree.length; ++i)
		{
	    	if (tree[i]['parent'] == root) 
	    	{
				...
				var li = jQuery("<li><table><tr><td><input type=\"hidden\" value=\"" + item['id'] + "\" /><img style=\"cursor:pointer\" width=\"10\" class=\"more-icon\" src=\""+expandCollapsePlus+"\"/><img style=\"cursor:pointer\" width=\"10\" class=\"less-icon\" src=\""+expandCollapseMinus+"\"/></td><td><span> " + item['label'] + " (" + item['count'] + ")</span></td></tr></table></li>");
				...
				if (jQuery(subtree).children().length > 0) {
					...
					jQuery(jQuery(li).find(".less-icon")[0]).show();
				}
				else {
